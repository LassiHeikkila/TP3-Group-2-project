                ...
                for (int i = 0; i < sysdat->columns; i++)
                {
                        ...
                        for (int j = start; j < sysdat->rows; j += 2)
                        {
                                ...
                                if (i == 0)
                                {
                                        if (j == 0)
                                        {
                                                u[i][j] = (1-rel_par)*pu[i][j] + rel_par*(1/3.0)*(u[i+1][j]+u[i][j+1]+u[i+1][j+1]);
                                        }
                                        ...
                                        {
                                                u[i][j] = (1-rel_par)*pu[i][j] + rel_par*((1/4.0)*(u[i+1][j]+u[i][j+1]+u[i][j-1])+(1/8.0)*(u[i+1][j+1]+u[i+1][j-1]));
                                ...
                                {
                                        u[i][j] = (1-rel_par)*pu[i][j] + rel_par*((1/4.0)*(u[i-1][j]+u[i+1][j]+u[i][j+1])+(1/8.0)*(u[i+1][j+1]+u[i-1][j+1]));
                                }
                                // Bottom edge:
                                else if (j == (sysdat->rows - 1))
                                {
                                ...
                                {
                                        u[i][j]=(1-rel_par)*pu[i][j]+rel_par*(0.2*(u[i-1][j]+u[i][j+1]+u[i][j-1]+u[i+1][j])+0.05*(u[i-1][j-1]+u[i-1][j+1]+u[i+1][j-1]+u[i+1][j+1]));
