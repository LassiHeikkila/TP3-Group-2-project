/*********************************************************************
The command line arguments go: filename.bmp, black, red, green, blue, 
desired convergence, max iterations.
*********************************************************************/
...
count=0;
        for ( unsigned int i = 0 ; i < d ; i++ ){
                for ( unsigned int j = 0 ; j < h ; j++ ){
                        ...
                        if (i == 0){
                                if (j == 0){
                                        u.array[i][j] = (1-s)*prevu.array[i][j]+s*0.5*(u.array[i+1][j]+u.array[i][j+1]);
                                }
                                else if (j == h - 1){
                                        u.array[i][j] = (1-s)*prevu.array[i][j]+s*0.5*(u.array[i+1][j]+u.array[i][j-1]);
                                }
                                else{
                                        u.array[i][j]=(1-s)*prevu.array[i][j]+s*(1/3.0)*(u.array[i+1][j]+u.array[i][j+1]+u.array[i][j-1]);
                                }
                       }
...
// Top edge:
                                ...
                                u.array[i][j]=(1-s)*prevu.array[i][j]+s*(1/3.0)*(u.array[i-1][j]+u.array[i+1][j]+u.array[i][j+1]);
                        }
// Bottom edge:
                        else if (j == (h - 1)){
                                u.array[i][j]=(1-s)*prevu.array[i][j]+s*(1/3.0)*(u.array[i-1][j]+u.array[i+1][j]+u.array[i][j-1]);
...
// Rest of area:
                                ...
                                u.array[i][j]=(1-s)*prevu.array[i][j]+s*0.25*(u.array[i+1][j]+u.array[i][j+1]+u.array[i-1][j]+u.array[i][j-1]);
                        }
