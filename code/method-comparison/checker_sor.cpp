// Dan Richardson, 26/2/15, checker_sor.cpp

// assesses convergence of checkerboard updating with sor to analytical solution
...
std::cout << "Checkboard (red-black) updating with over-relaxation\n";
std::cout << "----------------------------------------------------\n";
...
{
//set to zero at satrt of each loop
convergence = 0;
  ...
  for (j=1; j<=ny; j++)
  {
	//if point in circle, potential is 0
   	...
   	else
   	{
    		...
    		if ((i + j) % 2 == 0)
    		{
     			phi_new[i][j] = (1-omega) * phi[i][j] + omega/4 * (phi[i-1][j] + phi[i+1][j] + phi[i][j-1] + phi[i][j+1]);
    		}
   	}
  }
 }
 ...
 for (i=1; i<=nx; i++)
 {
  for (j=1; j<=ny; j++)
  {
	//odd points
   	if ((i + j) % 2 != 0)
   	{
		phi_new[i][j] = (1-omega) * phi[i][j] + omega/4 * (phi_new[i-1][j] + phi_new[i+1][j] + phi_new[i][j-1] + phi_new[i][j+1]);
   	}
  }
 }
...
for (j=0; j<=ny+1; j++)
{
 for (i=0; i<=nx+1; i++)
 {
  ...
  //open file
  std::ofstream file("data/checker_sor.dat", std::ios_base::app);
...
std::ofstream file1("data/checker_sor_time.dat", std::ios::out | std::ios::app);
std::ofstream file2("data/checker_sor_conv.dat", std::ios::out | std::ios::app);
std::ofstream file3("data/checker_sor_err.dat", std::ios::out | std::ios::app);
